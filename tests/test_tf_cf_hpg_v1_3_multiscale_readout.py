...
from research.candidates.tf_cf_hpg_v1_3_multiscale_readout import data
from research.candidates.tf_cf_hpg_v1_3_multiscale_readout.graph import (
    HybridGraphBuilder,
...
from research.candidates.tf_cf_hpg_v1_3_multiscale_readout.model import (
    CFHPGMultiscaleReadout,
    MaxRelativeGraphBlock,
    build_cf_hpg_v1_3_multiscale_readout,
    patchify_and_scale,
    pool_2x2_mean,
)
from research.candidates.tf_cf_hpg_v1_3_multiscale_readout import (
...
from research.candidates.tf_cf_hpg_v1_2_tokenizer import (
    train_validation_only as v1_2_training,
)


ROOT = Path(__file__).resolve().parents[1]
BASE = "032c38d49d189000b364d24af7e248eba200acf7"
CANDIDATE = ROOT / "research/candidates/tf_cf_hpg_v1_3_multiscale_readout"
...
def test_raw_tokenizer_has_exact_registered_order_before_positional_fusion():
    source = inspect.getsource(CFHPGMultiscaleReadout.call)
    ordered_fragments = (
...
def test_tokenizer_is_structurally_identical_to_v1_2():
    previous = build_cf_hpg_v1_2_tokenizer()
    candidate = build_cf_hpg_v1_3_multiscale_readout()
    previous_dense = {
        layer.name
    ...
    candidate_dense = {
        layer.name
        for layer in candidate._flatten_layers()
        if isinstance(layer, tf.keras.layers.Dense)
    }
    assert candidate_dense == previous_dense
    assert candidate.raw_projection.kernel.shape == previous.raw_projection.kernel.shape
    assert (
        candidate.raw_local_refinement.kernel.shape
        == previous.raw_local_refinement.kernel.shape
        == (96, 96)
    )
    assert candidate.raw_local_refinement.bias.shape == (96,)
...
def test_fine_graph_is_built_once_and_same_tensor_reused_twice(monkeypatch):
    model = build_cf_hpg_v1_3_multiscale_readout()
...
def test_model_output_shape_and_registered_stage_inventory():
    model = build_cf_hpg_v1_3_multiscale_readout()
    assert isinstance(model, CFHPGMultiscaleReadout)
    ...
    assert debug["fine_nodes"].shape == (3, 144, 96)
    assert debug["fine_mean"].shape == (3, 96)
    assert debug["fine_max"].shape == (3, 96)
    assert debug["fine_readout"].shape == (3, 192)
    assert debug["coarse_adjacency"].shape == (3, 36, 36)
    assert debug["coarse_nodes"].shape == (3, 36, 128)
    assert debug["coarse_mean"].shape == (3, 128)
    assert debug["coarse_max"].shape == (3, 128)
    assert debug["coarse_readout"].shape == (3, 256)
    assert debug["fused_readout"].shape == (3, 448)
...
def test_fine_coarse_and_fused_readouts_are_exact_registered_mean_max_order():
    model = build_cf_hpg_v1_3_multiscale_readout()
    images = tf.random.stateless_uniform([2, 48, 48, 1], [51, 3], maxval=255.0)
    _, debug = model(images, training=False, return_debug=True)

    fine_mean = tf.reduce_mean(debug["fine_nodes"], axis=1)
    fine_max = tf.reduce_max(debug["fine_nodes"], axis=1)
    coarse_mean = tf.reduce_mean(debug["coarse_nodes"], axis=1)
    coarse_max = tf.reduce_max(debug["coarse_nodes"], axis=1)
    fine_readout = tf.concat([fine_mean, fine_max], axis=-1)
    coarse_readout = tf.concat([coarse_mean, coarse_max], axis=-1)
    fused = tf.concat([fine_mean, fine_max, coarse_mean, coarse_max], axis=-1)

    np.testing.assert_array_equal(debug["fine_mean"], fine_mean)
    np.testing.assert_array_equal(debug["fine_max"], fine_max)
    np.testing.assert_array_equal(debug["fine_readout"], fine_readout)
    np.testing.assert_array_equal(debug["coarse_mean"], coarse_mean)
    np.testing.assert_array_equal(debug["coarse_max"], coarse_max)
    np.testing.assert_array_equal(debug["coarse_readout"], coarse_readout)
    np.testing.assert_array_equal(debug["fused_readout"], fused)
    ...
    ordered_fragments = (
        ...
        "coarse_nodes = nodes",
        "coarse_readout = tf.concat([coarse_mean, coarse_max], axis=-1)",
        "fused_readout = tf.concat([fine_readout, coarse_readout], axis=-1)",
        "pooled = self.readout_norm(pooled)",
        "pooled = self.readout_dense(pooled)",
    )
    offsets = [source.index(fragment) for fragment in ordered_fragments]
    assert offsets == sorted(offsets)
    assert source.count("tf.concat(") == 3


def test_only_readout_width_parameter_shapes_change_relative_to_v1_2():
    previous = build_cf_hpg_v1_2_tokenizer()
    candidate = build_cf_hpg_v1_3_multiscale_readout()

    def layer_variable_shapes(model):
        return {
            layer.name: tuple(
                tuple(variable.shape) for variable in layer.trainable_variables
            )
            for layer in model._flatten_layers()
            if layer is not model and layer.trainable_variables
        }

    previous_shapes = layer_variable_shapes(previous)
    candidate_shapes = layer_variable_shapes(candidate)
    assert previous_shapes.keys() == candidate_shapes.keys()
    changed = {
        name
        for name in previous_shapes
        if previous_shapes[name] != candidate_shapes[name]
    }
    assert changed == {"readout_norm", "readout_dense"}
    assert previous_shapes["readout_norm"] == ((256,), (256,))
    assert candidate_shapes["readout_norm"] == ((448,), (448,))
    assert previous_shapes["readout_dense"] == ((256, 128), (128,))
    assert candidate_shapes["readout_dense"] == ((448, 128), (128,))
    assert candidate.count_params() - previous.count_params() == 24_960
    assert len(candidate.trainable_variables) == len(previous.trainable_variables)
    assert len(candidate.variables) == len(previous.variables)


def test_parameter_budget_and_exact_variable_inventory():
    model = build_cf_hpg_v1_3_multiscale_readout()
    assert model.count_params() == training.EXPECTED_PARAMETER_COUNT == 445_799
    assert sum(int(tf.size(value)) for value in model.trainable_variables) == 445_799
...
def test_graph_and_data_sources_are_byte_identical_to_v1_2():
    for name in ("graph.py", "data.py"):
        v1_2 = (V1_2_CANDIDATE / name).read_bytes()
        candidate = (CANDIDATE / name).read_bytes()
        assert hashlib.sha256(candidate).digest() == hashlib.sha256(v1_2).digest()
...
def test_exact_future_optimizer_schedule_loss_and_lifecycle_config():
    assert training.STATUS == "CF_HPG_V1_3_MULTISCALE_READOUT_IMPLEMENTATION_ONLY"
    assert training.TRAINING_CONFIG == v1_2_training.TRAINING_CONFIG
...
@pytest.mark.parametrize(
    "candidate", [V1_0_CANDIDATE, V1_1_CANDIDATE, V1_2_CANDIDATE]
)
def test_v1_0_v1_1_and_v1_2_candidates_are_unchanged_from_base(candidate):
