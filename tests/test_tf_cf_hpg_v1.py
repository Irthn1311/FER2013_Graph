...
from research.candidates.tf_cf_hpg import data
from research.candidates.tf_cf_hpg.graph import (
    HybridGraphBuilder,
...
from research.candidates.tf_cf_hpg.model import (
    CFHPG,
...
from research.candidates.tf_cf_hpg import train_validation_only as training


ROOT = Path(__file__).resolve().parents[1]
BASE = "56a22a3200f1e0dc0100093d43f0db296c11487d"
CANDIDATE = ROOT / "research/candidates/tf_cf_hpg"
...
def test_patchification_is_exact_64_by_36_row_major_and_scaled():
    image = tf.reshape(tf.range(48 * 48, dtype=tf.float32), [1, 48, 48, 1])
    patches = patchify_and_scale(image).numpy()
    assert patches.shape == (1, 64, 36)
    expected_first = image.numpy()[0, :6, :6, 0].reshape(-1) / 127.5 - 1.0
    expected_second = image.numpy()[0, :6, 6:12, 0].reshape(-1) / 127.5 - 1.0
...
def test_coordinates_are_exact_grid_centers_in_row_major_xy_order():
    coordinates = normalized_grid_centers(8).numpy()
    axis = np.arange(8, dtype=np.float32) * 0.25 - 0.875
...
def test_exact_spatial_eight_neighborhood_has_no_self_and_is_bidirectional():
    adjacency = spatial_8_neighbor_adjacency(8).numpy()
    ...
    assert adjacency[9].sum() == 8
    assert np.flatnonzero(adjacency[0]).tolist() == [1, 8, 9]


def test_learned_relation_graph_has_exact_top_four_and_excludes_self():
    nodes = tf.random.stateless_normal([2, 16, 8], seed=[42, 1])
    adjacency = learned_relation_adjacency(nodes).numpy()
    assert adjacency.shape == (2, 16, 16)
...
def test_hybrid_graph_is_boolean_union_and_deduplicates_overlap():
    nodes = tf.random.stateless_normal([1, 16, 8], seed=[42, 2])
    spatial = spatial_8_neighbor_adjacency(4).numpy()
    learned = learned_relation_adjacency(nodes).numpy()[0]
    hybrid = HybridGraphBuilder(4)(nodes).numpy()[0]
...
def test_hierarchy_is_exact_parameter_free_two_by_two_arithmetic_mean():
    nodes = tf.reshape(tf.range(64, dtype=tf.float32), [1, 64, 1])
    pooled = pool_2x2_mean(nodes).numpy().reshape(4, 4)
    grid = np.arange(64, dtype=np.float32).reshape(8, 8)
    expected = np.asarray(
        [
            [grid[y : y + 2, x : x + 2].mean() for x in range(0, 8, 2)]
...
def test_coarse_graph_has_registered_spatial_and_top_four_semantics():
    nodes = tf.random.stateless_normal([2, 16, 128], seed=[42, 3])
    hybrid = HybridGraphBuilder(4, 4)(nodes).numpy()
    spatial = spatial_8_neighbor_adjacency(4).numpy()
...
def test_model_output_shape_and_registered_stage_inventory():
    ...
    assert debug["fine_adjacency"].shape == (3, 64, 64)
    assert debug["coarse_adjacency"].shape == (3, 16, 16)
    assert debug["final_nodes"].shape == (3, 16, 128)
...
def test_parameter_budget_and_exact_variable_inventory():
    model = build_cf_hpg_v1()
    assert model.count_params() == 413_447
    assert sum(int(tf.size(value)) for value in model.trainable_variables) == 413_447
    assert len(model.trainable_variables) == 64
    assert len(model.variables) == 74
...
@pytest.mark.parametrize(
    ("metrics", "expected"),
    [
        ((0.7000, 0.6700, 0.7800, 0.7500), "CF_HPG_V1_STRETCH_PASS"),
        ((0.6500, 0.6200, 0.7300, 0.7000), "CF_HPG_V1_PASS"),
        (
            (0.6500, 0.6200, 0.73001, 0.7000),
            "CF_HPG_V1_PERFORMANCE_WITH_GENERALIZATION_WARNING",
        ),
        ((0.6400, 0.6000, 0.7200, 0.6800), "CF_HPG_V1_NEAR_TARGET"),
        ((0.63999, 0.6000, 0.75001, 0.6800), "CF_HPG_V1_OVERFIT"),
        ((0.63999, 0.6000, 0.69999, 0.6800), "CF_HPG_V1_UNDERFIT"),
        ((0.6450, 0.6000, 0.7400, 0.6900), "CF_HPG_V1_INSUFFICIENT"),
    ],
)
...
def test_generation_one_and_step13_sources_are_unchanged_from_base():
    paths = [str(FROZEN.relative_to(ROOT)), *(str(path.relative_to(ROOT)) for path in STEP13_PATHS)]
