...
from research.candidates.tf_cf_hpg_v1_1_resolution import data
from research.candidates.tf_cf_hpg_v1_1_resolution.graph import (
    HybridGraphBuilder,
...
from research.candidates.tf_cf_hpg_v1_1_resolution import (
    train_validation_only as training,
)


ROOT = Path(__file__).resolve().parents[1]
BASE = "562c80fe675d8527416df1b5e441df76f1f62620"
CANDIDATE = ROOT / "research/candidates/tf_cf_hpg_v1_1_resolution"
...
def test_model_output_shape_and_registered_stage_inventory():
    model = build_cf_hpg_v1_1_resolution()
    assert isinstance(model, CFHPGResolution)
...
def test_parameter_budget_and_exact_variable_inventory():
    model = build_cf_hpg_v1_1_resolution()
    assert model.count_params() == training.EXPECTED_PARAMETER_COUNT == 411_527
    assert sum(int(tf.size(value)) for value in model.trainable_variables) == 411_527
...
def test_graph_and_data_sources_are_byte_identical_to_v1_0():
    for name in ("graph.py", "data.py"):
        v1_0 = (V1_0_CANDIDATE / name).read_bytes()
        resolution = (CANDIDATE / name).read_bytes()
        assert hashlib.sha256(resolution).digest() == hashlib.sha256(v1_0).digest()
...
@pytest.mark.parametrize(
    ...
    [
        ...
        ((0.5782808581777654, 0.50, 0.6150492876798217, 0.55), "RESOLUTION_STRONG_SIGNAL"),
        ((0.5582808581777654, 0.48, 0.58, 0.52), "RESOLUTION_PARTIAL_SIGNAL"),
        ((0.55, 0.45, 0.70, 0.60), "RESOLUTION_OVERFIT_SHIFT"),
        ((0.59, 0.50, 0.61, 0.54), "RESOLUTION_UNDERFIT_REMAINS"),
        ((0.61, 0.54, 0.60, 0.57), "RESOLUTION_INCONCLUSIVE"),
    ],
)
...
def test_v1_0_candidate_is_unchanged_from_base():
    completed = subprocess.run(
        ["git", "diff", "--quiet", BASE, "--", str(V1_0_CANDIDATE.relative_to(ROOT))],
        cwd=ROOT,
        check=False,
    )
