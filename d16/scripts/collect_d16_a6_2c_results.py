"""Collect D16R A6-2c main-logit pair-margin results."""
...
PAIR_COUNT_ANCHORS = {
    "A5b_seed42": {(2, 4): 109, (4, 2): 75, (4, 6): 94, (6, 4): 81},
    "A6_2b": {(2, 4): 105, (4, 2): 67, (4, 6): 108, (6, 4): 100},
}
...
def _micro_gate(run_dir: Path, prefix: str = "") -> Dict[str, Any]:
    ...
    if not source:
        return {
            "micro_gate_mean": float("nan"),
            "effective_motif_count_mean": float("nan"),
            "avg_offdiag_similarity_mean": float("nan"),
        }
    return {
...
def _artifact_status(run_dir: Path) -> Dict[str, Any]:
    ...
    return {"missing": missing, "integrity": "PASS" if not missing else "PARTIAL"}


def collect(run_dir: Path, output_dir: Path) -> Dict[str, Any]:
    artifact = _artifact_status(run_dir)
    missing = artifact["missing"]
    test = _first(run_dir / "test_metrics.csv")
    last = _first(run_dir / "last_test_metrics.csv")
    train_summary = _load_json(run_dir / "d16_train_summary.json")
    per_class = {
    ...
    if predicted_classes < 0:
        pred_rows_for_count = _read_rows(run_dir / "pred_count.csv")
        predicted_classes = sum(1 for row in pred_rows_for_count if _int(row.get("pred_count"), 0) > 0)
    best_epoch = _int(train_summary.get("best_epoch", test.get("checkpoint_epoch")))
    monitor = str(train_summary.get("best_monitor_metric", train_summary.get("checkpoint_monitor", "")))
        ...
        comparison_rows.append(
            {
                ...
                "a6_2c_minus_acc": accuracy - _float(anchor["accuracy"]),
                "a6_2c_minus_macro": macro_f1 - _float(anchor["macro_f1"]),
                "a6_2c_minus_hard_mean": "" if anchor.get("hard_mean", "") == "" else hard_mean - _float(anchor["hard_mean"]),
            }
        )
    comparison_rows.append(
        {
            ...
            "hard_mean": hard_mean,
            "a6_2c_minus_acc": 0.0,
            "a6_2c_minus_macro": 0.0,
            "a6_2c_minus_hard_mean": 0.0,
        }
    )
    ...
    for class_id in range(7):
        ...
        a5b_f1 = ANCHORS["A5b_seed42"].get(CLASS_NAMES[class_id], "")
        a6_2b_f1 = ANCHORS["A6_2b"].get(CLASS_NAMES[class_id], "")
        row["a5b_seed42_f1"] = a5b_f1
        row["a6_2b_f1"] = a6_2b_f1
        row["delta_f1_vs_a5b_seed42"] = "" if not isinstance(a5b_f1, float) else _float(row.get("f1")) - a5b_f1
        row["delta_f1_vs_a6_2b"] = "" if not isinstance(a6_2b_f1, float) else _float(row.get("f1")) - a6_2b_f1
    ...
    a6_2b_run = _find_run_dir("d16r_a6_2b_pairwise_relation_a5b_ce_seed42_accmon_150")
    confusion_maps = {"A6_2c": _confusion_map(run_dir / "confusion_matrix.csv")}
    ...
    if a6_2b_run is not None:
        confusion_maps["A6_2b"] = _confusion_map(a6_2b_run / "confusion_matrix.csv")
    ...
    for true_id, pred_id in PAIR_PATTERNS:
        ...
        for name, cmap in confusion_maps.items():
            ...
            row[f"{name}_count"] = item.get("count", PAIR_COUNT_ANCHORS.get(name, {}).get((true_id, pred_id), ""))
        ...
        loss_rows.append(
            {
                ...
                "pair_margin_loss_total": _float(row.get("pair_margin_loss_total")),
                "pair_margin_loss_fear_sad": _float(row.get("pair_margin_loss_fear_sad")),
                "pair_margin_loss_sad_neutral": _float(row.get("pair_margin_loss_sad_neutral")),
                "pair_margin_loss_neutral_sad": _float(row.get("pair_margin_loss_neutral_sad")),
                "lambda_pair_margin_current": _float(row.get("lambda_pair_margin_current")),
                "pair_count_fear_sad": _float(row.get("pair_count_fear_sad")),
                "pair_count_sad_neutral": _float(row.get("pair_count_sad_neutral")),
                "pair_count_neutral_sad": _float(row.get("pair_count_neutral_sad")),
                "mean_margin_violation_fear_sad": _float(row.get("mean_margin_violation_fear_sad")),
                "mean_margin_violation_sad_neutral": _float(row.get("mean_margin_violation_sad_neutral")),
                "mean_margin_violation_neutral_sad": _float(row.get("mean_margin_violation_neutral_sad")),
                "pair_margin_satisfied_fear_sad": _float(row.get("pair_margin_satisfied_fear_sad")),
                "pair_margin_satisfied_sad_neutral": _float(row.get("pair_margin_satisfied_sad_neutral")),
                "pair_margin_satisfied_neutral_sad": _float(row.get("pair_margin_satisfied_neutral_sad")),
    ...
    if accuracy < ANCHORS["D15"]["accuracy"]:
        decisions.append("REJECT_A6_2C")
    if accuracy > ANCHORS["A5b_seed42"]["accuracy"] and macro_f1 >= ANCHORS["A5b_mean"]["macro_f1"]:
        decisions.append("A6_2C_BEATS_A5B_SEED42")
    if accuracy >= ANCHORS["A5b_mean"]["accuracy"] and hard_mean > ANCHORS["A6_2b"]["hard_mean"]:
        decisions.append("A6_2C_USEFUL_MAIN_LOGIT_MARGIN")

    a6_2c_map = confusion_maps["A6_2c"]
    fs = a6_2c_map.get((2, 4), {}).get("count")
    sn = a6_2c_map.get((4, 6), {}).get("count")
    ns = a6_2c_map.get((6, 4), {}).get("count")
    a5b_map = confusion_maps.get("A5b_seed42", {})
    a5b_fs = a5b_map.get((2, 4), {}).get("count", PAIR_COUNT_ANCHORS["A5b_seed42"].get((2, 4)))
    a5b_sn = a5b_map.get((4, 6), {}).get("count", PAIR_COUNT_ANCHORS["A5b_seed42"].get((4, 6)))
    a5b_ns = a5b_map.get((6, 4), {}).get("count", PAIR_COUNT_ANCHORS["A5b_seed42"].get((6, 4)))
    if all(isinstance(v, int) for v in [fs, sn, ns, a5b_fs, a5b_sn, a5b_ns]):
        if fs <= a5b_fs - 5 and sn <= a5b_sn + 5 and ns <= a5b_ns + 5 and accuracy >= ANCHORS["A5b_mean"]["accuracy"] - 0.002:
            decisions.append("A6_2C_PAIRWISE_CONFUSION_IMPROVES")
    ...
    if (
        ...
        and (sad_f1 < ANCHORS["A5b_seed42"]["Sad"] - 0.015 or neutral_f1 < ANCHORS["A5b_seed42"]["Neutral"] - 0.015)
    ):
        decisions.append("PAIRWISE_MARGIN_TRADEOFF_NOT_SOLVED")
    if _float(micro.get("micro_gate_mean")) < 0.05:
        decisions.append("MAIN_LOGIT_MARGIN_SUPPRESSES_MICRO_SUPPORT")
    positive = {
        "A6_2C_BEATS_A5B_SEED42",
        "A6_2C_USEFUL_MAIN_LOGIT_MARGIN",
        "A6_2C_PAIRWISE_CONFUSION_IMPROVES",
    }
    if not any(decision in positive for decision in decisions):
        decisions.append("A6_2C_NOT_ENOUGH_STOP_AND_REANALYZE")
        decisions.append("STOP_A6_INCREMENTAL_TUNING_AND_REANALYZE")

    output_dir.mkdir(parents=True, exist_ok=True)
    _write_csv(
        output_dir / "d16r_a6_2c_comparison.csv",
        comparison_rows,
        ["run", "accuracy", "macro_f1", "hard_mean", "a6_2c_minus_acc", "a6_2c_minus_macro", "a6_2c_minus_hard_mean"],
    )
    _write_csv(
        output_dir / "d16r_a6_2c_per_class.csv",
        per_class_out,
        [
            "class_id",
            "class_name",
            ...
            "a5b_seed42_f1",
            "a6_2b_f1",
            "delta_f1_vs_a5b_seed42",
            "delta_f1_vs_a6_2b",
        ],
    )
    _write_csv(
        output_dir / "d16r_a6_2c_confusion_summary.csv",
    ...
    _write_csv(
        output_dir / "d16r_a6_2c_loss_diagnostics.csv",
        loss_rows,
        [
            "epoch",
            "train_loss",
    ...
    next_decision = {
        ...
        "a6_2b_confusion_source": "" if a6_2b_run is None else str(a6_2b_run),
        "decisions": decisions,
    }
    (output_dir / "d16r_a6_2c_next_decision.json").write_text(json.dumps(next_decision, indent=2), encoding="utf-8")

    hard_rows = [
        {
            "class": CLASS_NAMES[class_id],
            "A6_2c_f1": _float(per_class.get(class_id, {}).get("f1")),
            "A5b_seed42_f1": ANCHORS["A5b_seed42"][CLASS_NAMES[class_id]],
            "A6_2b_f1": ANCHORS["A6_2b"][CLASS_NAMES[class_id]],
            "delta_vs_A5b_seed42": _float(per_class.get(class_id, {}).get("f1")) - ANCHORS["A5b_seed42"][CLASS_NAMES[class_id]],
            "delta_vs_A6_2b": _float(per_class.get(class_id, {}).get("f1")) - ANCHORS["A6_2b"][CLASS_NAMES[class_id]],
        }
    ...
    report = f"""# D16R A6-2c Main-Logit Pair-Margin Analysis

## Executive Summary
Decision: `{", ".join(decisions)}`.

A6-2c keeps the A5b backbone, EdgeContextGNN, A4 micro-motif readout, node/edge features, classifier architecture, and inference path unchanged. The only intended change is a training-only, parameter-free margin loss on selected main-logit hard pairs: Fear->Sad, Sad->Neutral, and Neutral->Sad.
...
## Loss Diagnostics
{_markdown_table(loss_rows[-8:], ["epoch", "ce_loss", "pair_margin_loss_total", "pair_margin_loss_fear_sad", "pair_margin_loss_sad_neutral", "pair_margin_loss_neutral_sad", "lambda_pair_margin_current", "pair_margin_satisfied_fear_sad", "pair_margin_satisfied_sad_neutral", "pair_margin_satisfied_neutral_sad"])}
...
{_markdown_table(comparison_rows, ["run", "accuracy", "macro_f1", "hard_mean", "a6_2c_minus_acc", "a6_2c_minus_macro", "a6_2c_minus_hard_mean"])}

## Hard-Class F1 Comparison
{_markdown_table(hard_rows, ["class", "A6_2c_f1", "A5b_seed42_f1", "A6_2b_f1", "delta_vs_A5b_seed42", "delta_vs_A6_2b"])}

## Pairwise Confusion Comparison
{_markdown_table(pair_confusion_rows, ["pattern", "A6_2c_count", "A5b_seed42_count", "A6_2a_best_count", "A6_2a_last_diag_count", "A6_2b_count"])}
...
## Decision Notes
- Positive A6-2c signals require beating A5b seed42, improving over A5b mean with hard mean above A6-2b, or reducing targeted pair confusions without a Sad/Neutral trade-off.
- If none of those signals appear, stop incremental A6 tuning and reanalyze the hard-class route instead of stacking more auxiliary losses.

## Decision
`{", ".join(decisions)}`
"""
    _write_text(output_dir / "D16R_A6_2C_MAINLOGIT_PAIRMARGIN_ANALYSIS.md", report)
