...
def _write_report(path: Path, rows: List[Dict[str, Any]]) -> None:
    ...
    lines = [
        ...
        "",
        "## Results",
        "| name | graph_mode | batch | workers | status | epoch_min | memory_mb | node_mean | edge_mean |",
        "|---|---|---:|---:|---|---:|---:|---:|---:|",
    ]
    for row in rows:
        lines.append(
            "| {name} | {graph_mode} | {batch_size} | {num_workers} | {status} | {epoch_min} | {memory} | {node} | {edge} |".format(
                name=row.get("name"),
                ...
                node="" if row.get("node_count_mean") is None else f"{row.get('node_count_mean'):.1f}",
                edge="" if row.get("edge_count_mean") is None else f"{row.get('edge_count_mean'):.1f}",
            )
        )
    ...
    lines.append("- Prefer the fastest config that leaves comfortable GPU memory headroom and passes best-checkpoint evaluation contract.")
