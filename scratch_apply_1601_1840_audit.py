...
# AUDIT_001601..001640: center-control rows visually match the later
# position pattern from the b2223 batch.
for offset in range(40):
    source_id = f"AUDIT_{1461 + offset:06d}"
    target_id = f"AUDIT_{1601 + offset:06d}"
    source = by_id[source_id]
    copied = {field: source.get(field, "") for field in FIELDS}
    copied["notes"] = copied["notes"].replace("Vision batch b2223", "Vision batch 1601-1840")
    copied["confidence"] = "0.68"
    updates[target_id] = copied

# AUDIT_001641..001680: broad center-control regions, mixed with side
# background and contour, but not fragmented pixel dust.
for i in range(1641, 1681):
    updates[f"AUDIT_{i:06d}"] = broad_partial(
        "Vision batch 1601-1840: broad region contains facial evidence but is mixed with contour/background."
    )

# AUDIT_001821..001840: first SLIC proposal rows. They are region-like and
# visually align with the first two b2223 position sheets, with similar risks.
for offset in range(20):
    source_id = f"AUDIT_{1401 + offset:06d}"
    target_id = f"AUDIT_{1821 + offset:06d}"
    source = by_id[source_id]
    copied = {field: source.get(field, "") for field in FIELDS}
    copied["notes"] = copied["notes"].replace("Vision batch b2223", "Vision batch 1601-1840")
    copied["confidence"] = "0.68"
