...
CONFIG_PATH="configs/d16/main_branch/d16r_part_token_transformer_ce_seed42.yaml"
PRIOR_DIR="outputs/d16_mediapipe_pixel_priors_best_retry_rescue"
OUTPUT_DIR="outputs/d16_runs/main_branch/d16r_part_token_transformer_ce_seed42"
CHECK_DIR="outputs/d16_analysis/main_branch/d16r_part_token_transformer_ce_seed42_check"
ANALYSIS_DIR="outputs/d16_analysis/main_branch"

echo "[D16R-A2] Python/PyTorch/CUDA environment"
python - <<'PY'
import json
import platform
import sys
...
if [ ! -d "${PRIOR_DIR}" ]; then
  echo "[D16R-A2] Missing prior dir: ${PRIOR_DIR}" >&2
  exit 1
fi

if [ ! -f "${CONFIG_PATH}" ]; then
  echo "[D16R-A2] Missing config: ${CONFIG_PATH}" >&2
  exit 1
fi
...
do
  if [ ! -e "${OUTPUT_DIR}/${required}" ]; then
    echo "[D16R-A2] Missing required artifact: ${OUTPUT_DIR}/${required}" >&2
    exit 1
  fi
done
