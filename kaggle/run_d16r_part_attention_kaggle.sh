...
CONFIG_PATH="configs/d16/main_branch/d16r_part_attention_readout_ce_seed42.yaml"
PRIOR_DIR="outputs/d16_mediapipe_pixel_priors_best_retry_rescue"
OUTPUT_DIR="outputs/d16_runs/main_branch/d16r_part_attention_readout_ce_seed42"

echo "[D16R-A1] Python/PyTorch/CUDA environment"
python - <<'PY'
import json
import platform
import sys
...
if [ ! -d "${PRIOR_DIR}" ]; then
  echo "[D16R-A1] Missing prior dir: ${PRIOR_DIR}" >&2
  exit 1
fi

if [ ! -f "${CONFIG_PATH}" ]; then
  echo "[D16R-A1] Missing config: ${CONFIG_PATH}" >&2
  exit 1
fi
...
do
  if [ ! -e "${OUTPUT_DIR}/${required}" ]; then
    echo "[D16R-A1] Missing required artifact: ${OUTPUT_DIR}/${required}" >&2
    exit 1
  fi
done
...
python d16/scripts/check_d16_main_branch_run.py \
  --run_dir "${OUTPUT_DIR}" \
  --output_dir "outputs/d16_analysis/main_branch/d16r_part_attention_readout_ce_seed42_check"
