"""CF-HPG v1.3: fine+coarse multiscale readout with frozen graphs."""
...
@tf.keras.utils.register_keras_serializable(package="fer2013_graph_research")
class CFHPGMultiscaleReadout(tf.keras.Model):
    """Registered multiscale-readout Generation-2 hierarchical patch graph."""
    ...
    def call(self, images, training=False, return_debug=False):
        ...
        fine_nodes = nodes
        fine_mean = tf.reduce_mean(fine_nodes, axis=1)
        fine_max = tf.reduce_max(fine_nodes, axis=1)
...
def build_cf_hpg_v1_3_multiscale_readout() -> CFHPGMultiscaleReadout:
    model = CFHPGMultiscaleReadout(name="cf_hpg_v1_3_multiscale_readout")
