"""RA-HPG v1.0: relation-aware dynamic blocks on the frozen CF representation."""

from __future__ import annotations

import tensorflow as tf

from .graph import HybridGraphBuilder, normalized_grid_centers
...
def pool_2x2_mean(nodes: tf.Tensor) -> tf.Tensor:
    """Parameter-free 12x12 to 6x6 non-overlapping arithmetic-mean hierarchy."""
    ...
    grid = tf.reshape(
        nodes,
        [batch_size, COARSE_GRID_SIZE, 2, COARSE_GRID_SIZE, 2, width],
    )
    pooled = tf.reduce_mean(grid, axis=[2, 4])
    return tf.reshape(pooled, [batch_size, COARSE_NODE_COUNT, width])


def relative_edge_tensor(nodes: tf.Tensor) -> tf.Tensor:
    """Return the registered ordered relation tensor r_ij = z_j - z_i."""
...
def relation_aware_mean_aggregate(
    normalized_nodes: tf.Tensor,
    adjacency: tf.Tensor,
    relation_gate: tf.keras.layers.Layer,
) -> tuple[tf.Tensor, tf.Tensor, tf.Tensor]:
    """Compute degree-normalized sigmoid-gated relative-edge messages."""

    normalized_nodes = tf.convert_to_tensor(normalized_nodes)
    adjacency = tf.cast(tf.convert_to_tensor(adjacency), tf.bool)
    tf.debugging.assert_rank(normalized_nodes, 3)
    tf.debugging.assert_rank(adjacency, 3)
    tf.debugging.assert_equal(tf.shape(adjacency)[0], tf.shape(normalized_nodes)[0])
    tf.debugging.assert_equal(tf.shape(adjacency)[1], tf.shape(normalized_nodes)[1])
    tf.debugging.assert_equal(tf.shape(adjacency)[2], tf.shape(normalized_nodes)[1])
    tf.debugging.assert_equal(
        tf.reduce_all(tf.reduce_any(adjacency, axis=-1)),
        True,
        message="Every node requires at least one neighbor",
    )
    relative = relative_edge_tensor(normalized_nodes)
    gates = tf.math.sigmoid(relation_gate(relative))
    active = tf.cast(adjacency[:, :, :, None], normalized_nodes.dtype)
    degree = tf.reduce_sum(active, axis=2)
    message = tf.reduce_sum(active * gates * relative, axis=2)
    message = message / tf.maximum(degree, tf.ones_like(degree))
    return message, gates, relative


@tf.keras.utils.register_keras_serializable(package="fer2013_graph_research")
class RelationAwareDynamicGraphBlock(tf.keras.layers.Layer):
    """Registered pre-norm relation-aware graph block with a 4x FFN."""
    ...
    def __init__(self, width: int, dropout_rate: float = 0.15, **kwargs):
        ...
        self.relation_gate = tf.keras.layers.Dense(1, name="relation_gate")
        ...
        self.ffn_dense_in = tf.keras.layers.Dense(self.width * 4, name="ffn_dense_in")
    ...
    def call(self, nodes, adjacency, training=False, return_debug=False):
        normalized = self.graph_norm(nodes)
        message, gates, relative = relation_aware_mean_aggregate(
            normalized, adjacency, self.relation_gate
        )
        mixed = tf.concat([normalized, message], axis=-1)
        ...
        output = nodes + self.ffn_dropout(feed_forward, training=training)
        if return_debug:
            return output, {
...
@tf.keras.utils.register_keras_serializable(package="fer2013_graph_research")
class RAHPGRelationAware(tf.keras.Model):
    """Registered relation-aware bridge on the frozen CF-HPG v1.2 path."""

    def __init__(self, **kwargs):
        super().__init__(**kwargs)
        self.raw_projection = tf.keras.layers.Dense(FINE_WIDTH, name="raw_projection")
        self.raw_local_activation = tf.keras.layers.Activation(
            "gelu", name="raw_local_activation"
        )
        self.raw_local_refinement = tf.keras.layers.Dense(
            FINE_WIDTH, name="raw_local_refinement"
        )
        ...
        self.fine_graph_builder = HybridGraphBuilder(
            FINE_GRID_SIZE, 4, name="fine_hybrid_graph"
        )
        self.fine_blocks = [
            RelationAwareDynamicGraphBlock(
                FINE_WIDTH, 0.15, name=f"fine_block_{index + 1}"
            )
        ...
        self.coarse_graph_builder = HybridGraphBuilder(
            COARSE_GRID_SIZE, 4, name="coarse_hybrid_graph"
        )
        self.coarse_blocks = [
            RelationAwareDynamicGraphBlock(
                COARSE_WIDTH, 0.15, name=f"coarse_block_{index + 1}"
            )
            for index in range(2)
        ]
    ...
    def call(self, images, training=False, return_debug=False):
        ...
        positions = tf.broadcast_to(
            positions[None, :, :], [tf.shape(patches)[0], FINE_NODE_COUNT, 2]
        )
        ...
        for block in self.fine_blocks:
            fine_inputs.append(nodes)
            adjacency = self.fine_graph_builder(nodes)
            fine_adjacencies.append(adjacency)
            nodes = block(nodes, adjacency, training=training)
        ...
        for block in self.coarse_blocks:
            coarse_inputs.append(nodes)
            adjacency = self.coarse_graph_builder(nodes)
            coarse_adjacencies.append(adjacency)
            nodes = block(nodes, adjacency, training=training)

        coarse_readout = tf.concat(
            [tf.reduce_mean(nodes, axis=1), tf.reduce_max(nodes, axis=1)], axis=-1
        )
        pooled = self.readout_norm(coarse_readout)
        ...
        if return_debug:
            return logits, {
                "fine_block_inputs": tuple(fine_inputs),
                "fine_adjacencies": tuple(fine_adjacencies),
                "coarse_block_inputs": tuple(coarse_inputs),
                "coarse_adjacencies": tuple(coarse_adjacencies),
                "final_nodes": nodes,
                "coarse_readout": coarse_readout,
                "readout_hidden": pooled,
            }
        return logits
...
def build_ra_hpg_v1_relation_aware() -> RAHPGRelationAware:
    model = RAHPGRelationAware(name="ra_hpg_v1_relation_aware")
