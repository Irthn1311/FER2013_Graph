"""Future validation-only seed-42 lifecycle for RA-HPG v1.0."""
...
from .model import build_ra_hpg_v1_relation_aware
...
NUM_CLASSES = 7
EXPECTED_PARAMETER_COUNT = 626_987
EXPECTED_TRAINABLE_VARIABLE_COUNT = 74
EXPECTED_KERAS_VARIABLE_COUNT = 84
CF_HPG_V1_2_REFERENCE = {
...
def classify_outcome(
    *,
    ...
    delta_val_accuracy = round(deltas["delta_val_accuracy_pp"], 12)
    delta_clean_train_accuracy = round(
        deltas["delta_clean_train_accuracy_pp"], 12
    )
    if (
    ...
    if (
        ...
        return "RA_HPG_V1_PASS"
    if delta_clean_train_accuracy >= 5.0 and (
        acc_gap > 10.0 or macro_gap > 10.0
    ):
        return "RELATION_OVERFIT_SHIFT"
    if delta_val_accuracy >= 5.0 and delta_clean_train_accuracy >= 5.0:
        return "RELATION_STRONG_SIGNAL"
    if 3.0 <= delta_val_accuracy < 5.0:
        return "RELATION_PARTIAL_SIGNAL"
    if (
        delta_clean_train_accuracy >= 5.0
        and delta_val_accuracy < 3.0
        and acc_gap <= 10.0
        and macro_gap <= 10.0
    ):
        return "RELATION_FIT_WITHOUT_VAL_GAIN"
    if clean_train_accuracy < 0.7000 and validation_accuracy < 0.6200:
        return "RELATION_UNDERFIT_REMAINS"
    return "RELATION_INCONCLUSIVE"
...
def outcome_deltas(
    *,
    ...
    """Return the four preregistered percentage-point deltas from CF-HPG v1.2."""

    return {
        "delta_val_accuracy_pp": 100.0
        * (validation_accuracy - CF_HPG_V1_2_REFERENCE["validation_accuracy"]),
        "delta_val_macro_pp": 100.0
        * (validation_macro_f1 - CF_HPG_V1_2_REFERENCE["validation_macro_f1"]),
        "delta_clean_train_accuracy_pp": 100.0
        * (clean_train_accuracy - CF_HPG_V1_2_REFERENCE["clean_train_accuracy"]),
        "delta_clean_train_macro_pp": 100.0
        * (clean_train_macro_f1 - CF_HPG_V1_2_REFERENCE["clean_train_macro_f1"]),
    }
...
def validate_model_identity(model: tf.keras.Model) -> dict[str, int]:
    ...
    expected = {
        ...
        "trainable_variables": EXPECTED_TRAINABLE_VARIABLE_COUNT,
        "keras_variables": EXPECTED_KERAS_VARIABLE_COUNT,
    }
    if identity != expected:
        raise ValidationOnlyHarnessError(
            f"Registered architecture identity drift: {identity} != {expected}"
        )
    return identity
...
def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(
        description="Run the future registered RA-HPG v1.0 lifecycle."
    )
...
def main(argv: Sequence[str] | None = None) -> int:
    ...
    _atomic_json(
        output_root / "validation_only_result.json",
        {
            ...
            "decision": decision,
            "deltas_vs_cf_hpg_v1_2_pp": outcome_deltas(
                validation_accuracy=validation_metrics["accuracy"],
                validation_macro_f1=validation_metrics["macro_f1"],
                clean_train_accuracy=clean_metrics["accuracy"],
                clean_train_macro_f1=clean_metrics["macro_f1"],
