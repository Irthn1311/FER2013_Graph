...
def _seed(index, salt):
    return tf.random.experimental.stateless_fold_in(
        tf.stack([tf.constant(SEED, tf.int32), tf.cast(index, tf.int32)]), salt
    )


def sample_parameters(index):
    """Return the one deterministic registered parameter set for a sample."""

    flip = tf.random.stateless_uniform([], _seed(index, 0)) < 0.5
    angle = tf.random.stateless_uniform([], _seed(index, 1), minval=-10.0, maxval=10.0)
    translation = tf.random.stateless_uniform([2], _seed(index, 2), minval=-4.0, maxval=4.0)
    contrast = tf.random.stateless_uniform([], _seed(index, 3), minval=0.85, maxval=1.15)
    brightness = tf.random.stateless_uniform([], _seed(index, 4), minval=-0.10, maxval=0.10)
    erase = tf.random.stateless_uniform([], _seed(index, 5)) < 0.25
    area = tf.random.stateless_uniform([], _seed(index, 6), minval=0.02, maxval=0.10)
    aspect = tf.random.stateless_uniform([], _seed(index, 7), minval=0.5, maxval=2.0)
    return {
        "flip": flip,
...
def _erase(image, index, parameters):
    ...
    top = tf.random.stateless_uniform([], _seed(index, 8), 0, 48 - height + 1, dtype=tf.int32)
    left = tf.random.stateless_uniform([], _seed(index, 9), 0, 48 - width + 1, dtype=tf.int32)
...
def augment_example(raw_image, support, sample_index, *, return_debug=False):
    ...
    image = tf.cond(parameters["erase"], lambda: _erase(image, sample_index, parameters), lambda: image)
