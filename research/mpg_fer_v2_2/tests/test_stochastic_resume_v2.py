...
from mpg_fer_v2_2.checkpoint import (
    atomic_save_resume,
    build_resume_bundle,
    load_resume_bundle,
...
from mpg_fer_v2_2.data import EpochRandomSampler
from mpg_fer_v2_2.ema import ModelEMA
from mpg_fer_v2_2.losses import supervised_contrastive_loss, symmetric_js_divergence
from mpg_fer_v2_2.model import DropPath
from mpg_fer_v2_2.motif import SpatialMotifComposer
...
class TinyActualV21(nn.Module):
    """Compact harness using actual v2.2 motif, tau, MI, DropPath and SupCon."""
...
def test_actual_v2_2_stochastic_multiworker_trajectory_is_resume_invariant(tmp_path) -> None:
    ...
    bundle = build_resume_bundle(
        config=config, run_id="actual-v2-2", source_hash="actual-source",
    ...
    loaded = load_resume_bundle(
        ...
        run_id="actual-v2-2", source_hash="actual-source",
    )
    restore_training_state(
...
def test_actual_v2_2_epoch_84_to_85_resume_boundary_is_invariant(tmp_path) -> None:
    config = MPGConfig(gradient_accumulation_steps=1)
    all_epochs = [84, 85, 86]
    first_epochs = [84]
