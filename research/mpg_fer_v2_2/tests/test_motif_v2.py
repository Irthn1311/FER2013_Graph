...
from mpg_fer_v2_2.losses import motif_mutual_information_loss, symmetric_js_divergence
from mpg_fer_v2_2.model import GeometryAwareMotifTransformerBlock, compute_motif_geometry
from mpg_fer_v2_2.motif import (
...
def test_temperature_schedule_and_serialized_buffer() -> None:
    composer = SpatialMotifComposer()
    assert "raw_tau" not in dict(composer.named_parameters())
    assert "current_tau" in dict(composer.named_buffers())
    expected = {1: 0.70, 35: 0.30, 36: 0.30, 120: 0.30}
    for epoch, value in expected.items():
        assert scheduled_motif_temperature(epoch) == pytest.approx(value)
        assert composer.set_epoch_temperature(epoch) == pytest.approx(value)
        assert float(composer.temperature) == pytest.approx(value)
    clone = SpatialMotifComposer()
    clone.load_state_dict(composer.state_dict(), strict=True)
    assert float(clone.temperature) == pytest.approx(0.30)
...
def test_scheduled_temperature_is_copied_not_averaged_by_ema() -> None:
    from mpg_fer_v2_2.ema import ModelEMA

    composer = SpatialMotifComposer(d_pixel=12, d_type=4, d_motif=16)
    ema = ModelEMA(composer, decay=0.5)
    composer.set_epoch_temperature(35)
    ema.update(composer)
    assert float(composer.temperature) == pytest.approx(0.30)
    assert float(ema.module.temperature) == pytest.approx(0.30)
