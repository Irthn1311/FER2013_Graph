...
from mpg_fer_v2_1.checkpoint import config_hash
from mpg_fer_v2_1.kaggle import resolve_resume_artifact
...
def test_v2_1_training_contract_and_scheduler() -> None:
    config = MPGConfig()
    assert (
        config.max_epochs,
...
def test_resume_auto_never_discovers_v1_v2_or_best_checkpoint(tmp_path) -> None:
...
def test_resume_auto_finds_nested_explicitly_named_v2_1_dataset(tmp_path) -> None:
    import hashlib
    nested = tmp_path / "datasets" / "owner" / "mpg-fer-v2-1-resume-run-a"
...
def test_corrupt_latest_reports_but_does_not_auto_load_valid_snapshot(tmp_path) -> None:
    import hashlib
    nested = tmp_path / "mpg-fer-v2-1-resume-run-b"
    nested.mkdir()
...
def test_generated_notebook_matches_all_reviewed_sources() -> None:
    ...
    actual = json.loads((ROOT / "notebooks" / "MPG_FER_v2_1_Kaggle_T4.ipynb").read_text(encoding="utf-8"))
    ...
    assert "RESUMING EXISTING MPG-FER v2.1 RUN" in code
