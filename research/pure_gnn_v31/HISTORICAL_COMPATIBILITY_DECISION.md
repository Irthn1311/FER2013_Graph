...
3. **Base Optimizer Family & Initial Learning Rate**:
   - Optimizer: AdamW.
   - Initial Learning Rate: $3 \times 10^{-4}$ ($0.0003$).
   - *Status*: **SOURCE_CONFIRMED**.
4. **Validation Frequency & Model Selection Metric**:
   ...
   - Save-best-only: True (`best_val_accuracy.keras` or `.weights.h5`).
   - *Status*: **SOURCE_CONFIRMED**.
5. **Early Stopping Metric & Patience**:
   - Monitor: `val_loss`, mode `min`.
   - Patience: 15 epochs.
   - *Status*: **SOURCE_CONFIRMED**.
6. **Canonical Reproducibility Seed**:
...
### 3.1 Pixel Input Normalization (DIVERGENT - NOT SHARED)
- **LAP OFIX7-mid**: `standalone/lap_gnn_tensorflow_ofix7_mid_candidate/src/lap_gnn_tf/graph/builder.py:569` uses `image / 255.0 if image.max() > 1.0 else image`. Note that `fer2013.py` only validates CSV headers/counts and does not perform pixel scaling.
- **Gen2 CF-HPG**: `research/candidates/tf_cf_hpg_v1_1_resolution/data.py` loads raw pixels without `/255`. Then `model.py` (`patchify_and_scale()`) executes `images = images / 127.5 - 1.0`, scaling raw $[0, 255] \to [-1.0, 1.0]$.
- **Gen3 WS-HPG**: `research/candidates/tf_ws_hpg_v1_training/data.py:35` validation path uses `image / 255.0`, and `augmentation.py:59` training path begins with `raw_image / 255.0`, scaling to $[0.0, 1.0]$.
- **Pure-GNN Specific Contract**: Pure-GNN scientific loader uses `raw / 255.0` $\implies [0.0, 1.0]$. This is designated as **`PURE_GNN_SPECIFIC_INPUT_CONTRACT`** (derived from `research/pure_gnn_v31/src/pure_gnn_v31/scientific/dataset.py`). It is NOT a shared historical consensus.

### 3.2 Data Augmentation Policy (DIVERGENT)
- **LAP OFIX7-mid**: Image augmentation is **UNKNOWN** (no executable crop/flip logic found in `lap_gnn_tf` or baseline configs). Structural prior corruption is **SOURCE_CONFIRMED** (`baseline.yaml:58-77`, modes: `attenuate_prior`, `shuffle_prior`, `zero_prior`, `forced_fallback`).
...
- **Pure-GNN Status**: Pure-GNN currently tests `paired_sample_ordering` only. `paired_augmentation_status = "BLOCKED_ON_AUGMENTATION_POLICY"`.
- **Decision**: `augmentation_policy = null`, `status = "REQUIRES_REVIEW"`.

### 3.3 Batch Size (LAP 16 vs Gen2/Gen3 64)
...
### 3.4 Learning Rate Scheduler (ReduceLROnPlateau vs WarmupCosine)
...
### 3.5 Weight Decay (0.001 vs 0.0005)
...
### 3.6 Max Training Epochs (90 vs 100)
...
### 3.7 Label Smoothing (0.0 vs 0.05)
...
| `pixel_normalization` | Divergent (LAP: [0, 1] if max>1, Gen2: [-1, 1], Gen3: [0, 1]) | `"raw_div_255"` | **PURE_GNN_SPECIFIC_INPUT_CONTRACT** |
