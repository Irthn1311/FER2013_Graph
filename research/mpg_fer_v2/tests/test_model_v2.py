...
from mpg_fer_v2.model import DropPath, EdgeAwarePixelGNNLayer, MPGFER, masked_neighbor_softmax


def test_v2_parameter_count_is_small_delta_from_v1() -> None:
    parameters = sum(p.numel() for p in MPGFER().parameters() if p.requires_grad)
    assert parameters == 2_238_609
    assert parameters / 2_219_788 < 1.01
