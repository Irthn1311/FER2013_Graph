...
class AuditTrainingModel(nn.Module):
    def __init__(self) -> None:
        ...
        self.raw_tau = nn.Parameter(torch.tensor(0.2))

    def forward(self, images):
        logits = self.classifier(images) + self.raw_tau
        zero = self.raw_tau.square() * 0.01
        outputs = {
            "pixel_logits": logits,
            "motif_logits": logits,
            "loss_diversity": zero,
        ...
        outputs.update({name: self.raw_tau for name in MOTIF_DIAGNOSTICS})
...
def _objects(config: MPGConfig):
    ...
    scheduler = WarmupCosineScheduler(optimizer, config.learning_rate, 1, 4, config.min_learning_rate)
...
def test_full_resume_round_trip_restores_all_declared_state(tmp_path) -> None:
    config = MPGConfig(max_epochs=4, warmup_epochs=1)
