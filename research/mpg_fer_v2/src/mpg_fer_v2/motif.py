"""Differentiable multiscale spatial motif composition for MPG-FER v2."""
...
def build_aligned_support_indices(
    ...
    if len(starts) != 7:
        raise ValueError("MPG-FER v2 requires a 7x7 anchor grid")
    centers = []
...
class SpatialMotifComposer(nn.Module):
    ...
    def __init__(
        ...
        super().__init__()
        if not tau_min < tau_init < tau_max:
            raise ValueError("tau_init must be strictly inside [tau_min,tau_max]")
        ...
        initial_fraction = (tau_init - tau_min) / (tau_max - tau_min)
        self.raw_tau = nn.Parameter(torch.tensor(math.log(initial_fraction / (1.0 - initial_fraction))))
