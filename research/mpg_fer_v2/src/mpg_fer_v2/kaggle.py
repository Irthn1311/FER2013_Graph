"""Fail-closed Kaggle data and explicit MPG-FER v2 resume resolution."""
...
def resolve_resume_artifact(
    ...
    starts with ``mpg-fer-v2-resume``. It cannot discover v1 checkpoints or a
    ...
    if explicit_path is not None:
        ...
        candidates = sorted(
            ...
            if any(
                part.startswith("mpg-fer-v2-resume")
                for part in path.relative_to(root).parts[:-1]
            )
        ) if root.exists() else []
    if not candidates:
        if normalized == "required":
            raise FileNotFoundError("No explicit MPG-FER v2 resume artifact attached")
        return None, None
    if len(candidates) != 1:
