"""Pure-GNN MPG-FER v2 model: pixel graph, motifs, and motif graph."""
...
class MPGFER(nn.Module):
    """MPG-FER v2; no convolutional or dense-image transformer backbone."""
    ...
    def __init__(self, config: MPGConfig | None = None) -> None:
        ...
        self.motif_composer = SpatialMotifComposer(
            ...
            tau_min=cfg.tau_min, tau_max=cfg.tau_max, tau_init=cfg.tau_init,
            mi_beta=cfg.mi_beta,
        )
    ...
    def forward(self, x: torch.Tensor) -> tuple[torch.Tensor, dict[str, torch.Tensor]]:
        ...
        logits = self.classifier(torch.cat([pixel_readout, motif_readout], dim=-1))
        outputs = {
            "final_logits": logits,
