"""Generate the source-locked fresh/resume MPG-FER v2.3 Kaggle notebook."""
...
SOURCE_ROOT = PROJECT_ROOT / "src" / "mpg_fer_v2_3"
NOTEBOOK_PATH = PROJECT_ROOT / "notebooks" / "MPG_FER_v2_3_Kaggle_T4.ipynb"
SOURCE_FILES = (
...
def build_notebook() -> dict:
    ...
    bootstrap = f'''# Generated from the reviewed GitHub Issue #97 protocol. Do not hand-edit embedded code.
import sys
from pathlib import Path

EMBEDDED_SOURCES = {sources!r}
EMBEDDED_ROOT = Path("/kaggle/working/mpg_fer_v2_3_embedded")
EMBEDDED_PACKAGE = EMBEDDED_ROOT / "mpg_fer_v2_3"
...
print(f"Materialized reviewed MPG-FER v2.3 package at {{EMBEDDED_PACKAGE}}")
'''
    cells = [
        _markdown(
            "# MPG-FER v2.3 — source-locked segmented Kaggle T4 candidate\n\n"
            "Generated from the preregistered GitHub Issue #97 protocol on the exact "
            "reviewed v2.2 base. The scientific package is "
...
SEGMENT_NUMBER = 1
OUTPUT_DIR = "/kaggle/working/mpg_fer_v2_3_run"
ACCOUNT = "irthn1311"
KERNEL_REF = "irthn1311/mpg-fer-v2-3-t4-review-candidate"
...
'''
        ...
        _code(bootstrap),
        _code(
            '''# Environment, source/model/data gates, and explicit v2.3-only resume resolution.
...
from mpg_fer_v2_3.checkpoint import config_hash, load_resume_bundle, sha256_file
from mpg_fer_v2_3.config import MPGConfig
from mpg_fer_v2_3.data import FER2013Dataset, validate_dataset_gate
from mpg_fer_v2_3.ema import ModelEMA
from mpg_fer_v2_3.kaggle import resolve_kaggle_splits, resolve_resume_artifact
from mpg_fer_v2_3.model import MPGFER
from mpg_fer_v2_3.train import (
...
if re.fullmatch(r"[0-9a-f]{40}", GIT_COMMIT_SHA) is None:
    raise RuntimeError(
        "INVALID_GIT_COMMIT_SHA: stage the notebook with an exact lowercase 40-hex commit"
    )
os.environ["MPG_FER_SOURCE_GIT_COMMIT"] = GIT_COMMIT_SHA
...
if cfg.motif_topk_schedule != (8, 16, 16, 16, 24):
    raise RuntimeError(f"V23_TOPK_CONTRACT_MISMATCH: {cfg.motif_topk_schedule}")
if cfg.motif_residual_scale_schedule != (0.5, 0.5, 1.0, 1.0, 1.0):
    raise RuntimeError(
        "V23_RESIDUAL_SCALE_CONTRACT_MISMATCH: "
        f"{cfg.motif_residual_scale_schedule}"
    )
...
# Official v2.3 preserves the reviewed v2.2 physical-batch contract.
...
if resume_checkpoint is None:
    try:
        ...
        raise RuntimeError(
            "BATCH16_OOM: official v2.3 requires physical batch 16 and "
...
'''
    ...
    return {
        ...
        "metadata": {
            ...
            "mpg_fer_v2_3_source_manifest": hashes,
            "mpg_fer_v2_3_issue": 97,
