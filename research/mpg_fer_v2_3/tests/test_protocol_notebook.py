...
from mpg_fer_v2_3.kaggle import resolve_resume_artifact
...
def test_v2_3_preserves_training_contract_and_scheduler() -> None:
    config = MPGConfig()
    assert (
        config.max_epochs,
...
def test_resume_auto_finds_nested_explicitly_named_v2_3_dataset(tmp_path) -> None:
    import hashlib
    nested = tmp_path / "datasets" / "owner" / "mpg-fer-v2-3-resume-run-a"
...
def test_corrupt_latest_reports_but_does_not_auto_load_valid_snapshot(tmp_path) -> None:
    import hashlib
    nested = tmp_path / "mpg-fer-v2-3-resume-run-b"
    nested.mkdir()
...
def test_generated_notebook_matches_all_reviewed_sources() -> None:
    ...
    actual = json.loads((ROOT / "notebooks" / "MPG_FER_v2_3_Kaggle_T4.ipynb").read_text(encoding="utf-8"))
    ...
    assert "preregistered GitHub Issue #97 protocol" in all_text
    assert 'os.environ["MPG_FER_SOURCE_GIT_COMMIT"] = GIT_COMMIT_SHA' in code
    assert "INVALID_GIT_COMMIT_SHA" in code
    assert "V23_TOPK_CONTRACT_MISMATCH" in code
    assert "V23_RESIDUAL_SCALE_CONTRACT_MISMATCH" in code
    assert actual["metadata"]["mpg_fer_v2_3_issue"] == 97
