"""Fail-closed Kaggle data and explicit MPG-FER v2.3 resume resolution."""
...
def resolve_resume_artifact(
    ...
    """Resolve only an explicit or clearly named v2.3 resume attachment.

    Auto-discovery is deliberately restricted to datasets whose directory name
    starts with ``mpg-fer-v2-3-resume``. It cannot discover v1/v2/v2.1
    ...
    if explicit_path is not None:
        ...
        candidates = sorted(
            ...
            if any(
                part.startswith("mpg-fer-v2-3-resume")
                for part in path.relative_to(root).parts[:-1]
            )
        ) if root.exists() else []
    if not candidates:
        if normalized == "required":
            raise FileNotFoundError("No explicit MPG-FER v2.3 resume artifact attached")
        return None, None
    if len(candidates) != 1:
        raise RuntimeError(
            f"Expected exactly one v2.3 resume artifact, found {len(candidates)}"
        )
    checkpoint = candidates[0]
    if checkpoint.name != "resume_latest.pt" or not checkpoint.is_file():
        raise RuntimeError(f"Invalid v2.3 resume checkpoint path: {checkpoint}")
