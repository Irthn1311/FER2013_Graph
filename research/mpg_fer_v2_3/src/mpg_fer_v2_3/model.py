"""Pure-GNN MPG-FER v2.3 model: pixel graph, motifs, and motif graph."""
...
class GeometryAwareMotifTransformerBlock(nn.Module):
    def __init__(
        ...
        if (
            ...
            raise ValueError("residual_scale must be numeric in (0, 1]")
    ...
    def forward(
        ...
        h_motif = h_motif + self.residual_scale * self.drop_path1(
            self.out_proj(message)
        )
        h_motif = h_motif + self.residual_scale * self.drop_path2(
            self.ffn(self.norm2(h_motif))
        )
        ...
        layer_diag = {
            ...
            "residual_scale": torch.tensor(
                self.residual_scale, dtype=h_motif.dtype, device=h_motif.device
            ),
...
class MPGFER(nn.Module):
    """MPG-FER v2.3; no convolutional or dense-image transformer backbone."""
    ...
    def __init__(self, config: MPGConfig | None = None) -> None:
        ...
        self.motif_gnn = nn.ModuleList([
            GeometryAwareMotifTransformerBlock(
                ...
                residual_scale=cfg.motif_residual_scale_schedule[i],
            )
    ...
    def model_summary(
        ...
        *,
        ...
        """Return the machine-readable v2.3 construction identity."""
        return {
            ...
            "early_depth_intervention": {
                ...
                "motif_residual_scale_schedule": list(
                    self.config.motif_residual_scale_schedule
                ),
                "target_layers": [1, 2],
                "coefficient_sweep": False,
            },
            "motif_topk_schedule": list(self.config.motif_topk_schedule),
            "source_git_commit": source_git_commit or "UNKNOWN",
            "source_sha256": source_sha256 or "UNKNOWN",
        }
