...
`protocol.py` requires the resolved `training` and `loss` mappings and the seed
to equal that baseline. It does not use the batch32 Kaggle configuration.
...
This runner uses the same single-device training loop as the frozen reference;
it does not multiply the batch size or add a new distributed protocol.
...
```python
!python run_pixel_gnn.py --smoke --fer-csv /kaggle/input/YOUR_DATASET/fer13-split/train.csv
```

Or check architecture/gradients explicitly using synthetic data:

```python
!python run_pixel_gnn.py --smoke --synthetic
```

Then run the registered full protocol:

```python
!python run_pixel_gnn.py --fer-csv /kaggle/input/YOUR_DATASET/fer13-split/train.csv
```
...
Outputs are separate, under `/kaggle/working/outputs/pixel_gnn_only_seed42`
and `/kaggle/working/outputs/pixel_gnn_only_smoke`. Existing nonempty output
directories are rejected; use `--output-root` with a new directory for another run.
