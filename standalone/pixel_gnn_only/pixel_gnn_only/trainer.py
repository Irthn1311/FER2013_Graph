"""Isolated ablation loop copied from frozen reference at 62db5a6.

Only model/data/signature adapters differ. Selection and scheduler ordering
are retained; the frozen reference trainer itself is never edited.
"""
...
def run_training(
    ...
    val_data = GraphBatchGenerator(
        fer_csv, "val", config, controls.eval_batch_size, seed, False,
        controls.graph_cache_size, telemetry, graph_workers=controls.graph_workers,
        clean_graph_cache_dir=controls.clean_graph_cache_dir,
    )
    eval_step = None
    ...
    write_pixel_provenance(output_dir, [train_data.dataset, val_data.dataset])
    history = []
    ...
    artifact_paths = [
        ...
        output_dir / "architecture_report.json",
        output_dir / "pixel_inputs.json",
