...
Architecture:
1. Input: 48x48 pixel graph -> 2304 nodes, 5 channels [I, x_norm, y_norm, gx, gy]
2. Projection: Linear(5 -> D) + GELU + Dropout
3. Local Neighbor Attention: 1 or 2 layers on static 8-neighborhood (no CNN, no global transformer)
4. Learned Motif Prototypes: K trainable prototype vectors in R^D (default K=32)
5. Soft Assignment: cosine similarity normalized by temperature tau (default 0.1) -> [B, 2304, K]
6. Motif Aggregation: weighted mean of node embeddings assigned to each motif -> [B, K, D]
7. Motif Attention Pooling: lightweight attention over K motifs -> [B, D]
8. Classifier: Linear(D -> 7) -> logits for 7 FER2013 classes
"""
...
class LocalNeighborAttentionLayer(tf.keras.layers.Layer):
    ...
    def call(self, h, training: bool = False):
        """
        Args:
            h: Node embeddings of shape [B, 2304, D]
        Returns:
            Updated node embeddings of shape [B, 2304, D]
        """
...
class LearnedMotifPrototypeLayer(tf.keras.layers.Layer):
    ...
    def build(self, input_shape):
        # K trainable prototype vectors in R^D
    ...
    def call(self, h):
        ...
        Args:
            h: Node embeddings of shape [B, 2304, D]
        Returns:
            motif_reps: Aggregated motif vectors [B, K, D]
            assignment: Soft assignment weights [B, 2304, K]
            prototypes: Normalized prototypes [K, D]
        """
        # Normalize node embeddings and prototypes
        ...
        # Cosine similarity: [B, 2304, K]
        similarity = tf.einsum("bnd,kd->bnk", h_norm, p_norm)

        # Soft assignment over K motifs
        assignment = tf.nn.softmax(similarity / self.temperature, axis=-1)  # [B, 2304, K]

        # Motif aggregation: weighted average of node embeddings
        # M_unnorm = sum_n (A_bnk * H_bnd) -> [B, K, D]
        motif_unnorm = tf.einsum("bnk,bnd->bkd", assignment, h)

        # Denominator: sum of assignment weights per motif -> [B, K, 1]
...
class MotifAttentionPooling(tf.keras.layers.Layer):
    ...
    def call(self, motif_reps):
        ...
        Returns:
            graph_embedding: [B, D]
            beta: Motif attention weights [B, K]
        """
        # Linear score for each motif: [B, K, 1]
        scores = self.score_linear(motif_reps)

        # Softmax over K motifs: [B, K, 1]
...
@tf.keras.utils.register_keras_serializable(package="pixel_neighbor_motif")
class PixelNeighborMotifModel(tf.keras.Model):
    ...
    def __init__(
        ...
        **kwargs,
        ...
        # 1. Pixel Node projection: Linear(5 -> hidden_dim)
        ...
        # 2. Local Neighbor Attention layers
        ...
        # 3. Readout mechanics (Motif vs Global Mean)
    ...
    def call(self, batch, training: bool = False):
        ...
        Args:
            batch: dict with:
                - "node_features": [B, 2304, 5] (or [B*2304, 5])
        """
        x = batch["node_features"]

        # Ensure batch is 3D: [B, 2304, 5]
        if len(x.shape) == 2:
            x = tf.reshape(x, [-1, 2304, 5])

        # Step 1: Linear projection + GELU
        h = tf.nn.gelu(self.node_proj(x), approximate=False)
        if training and self.dropout_rate > 0.0:
            h = self.node_dropout(h, training=training)

        # Step 2: Neighbor Attention
        for att_layer in self.attention_layers:
            h = att_layer(h, training=training)

        node_embeddings = h  # [B, 2304, D]

        # Step 3: Pooling (Motif or Global Mean)
        if self.pooling_type == "motif":
            motif_reps, assignment, p_norm = self.motif_layer(node_embeddings)
            z_image, beta = self.motif_pooling(motif_reps)
            motif_usage = tf.reduce_mean(assignment, axis=[0, 1])  # [K]
        else:
            # Baseline ablation: Global Mean Pooling
            z_image = tf.reduce_mean(node_embeddings, axis=1)      # [B, D]
            assignment = None
            p_norm = None
            beta = None
            motif_usage = None
        ...
        logits = self.classifier(z_image)                          # [B, 7]
