...
from lap_gnn.data.graph_builder import D16GraphData, build_pixel_graph
from lap_gnn.data.mediapipe_priors import fallback_priors
...
class D16PixelPriorDataset(Dataset):
    ...
    def _zero_prior(prior: Dict[str, np.ndarray]) -> Dict[str, np.ndarray]:
        out = D16PixelPriorDataset._copy_prior(prior)
    ...
    def _shuffle_prior(base: Dict[str, np.ndarray], donor: Dict[str, np.ndarray]) -> Dict[str, np.ndarray]:
        out = D16PixelPriorDataset._copy_prior(base)
        for key in (
            "face_mask",
            "part_soft_masks",
            "micro_anchor_maps",
    ...
    def _forced_fallback(prior: Dict[str, np.ndarray]) -> Dict[str, np.ndarray]:
        ...
        return D16PixelPriorDataset._copy_prior(result.arrays)
