...
from lap_gnn.data.graph_builder import D16Batch, collate_d16_graphs
from lap_gnn.data.graph_cache_dataset import D16GraphCacheDataset
from lap_gnn.data.pixel_prior_dataset import D16PixelPriorDataset
from lap_gnn.losses.hard_proto_separation import (
    HardPrototypeSeparationLoss,
    build_hard_proto_separation_loss,
    hard_proto_lambda,
)
from lap_gnn.losses.main_logit_pair_margin import (
    MainLogitPairMarginLoss,
    build_main_logit_pair_margin_loss,
    main_logit_pair_margin_lambda,
)
from lap_gnn.losses.part_supcon import PartAwareSupConLoss
from lap_gnn.losses.pairwise_hard_relation import (
...
from lap_gnn.config import canonical_hash
from lap_gnn.constants import PARENT_COMMIT, SIGNATURES
from lap_gnn.signatures import sha256_file
...
def save_checkpoint(
    ...
    payload = {
        ...
        "dataset_signature": SIGNATURES["dataset"],
        "split_signature": SIGNATURES["split"],
        "prior_schema_signature": SIGNATURES["prior_schema"],
        "seed": int(config.get("seed", (config.get("training", {}) or {}).get("seed", -1))),
        "framework": "pytorch",
        "framework_version": torch.__version__,
        "parent_source_commit": PARENT_COMMIT,
        "package_checksum": (
            sha256_file(Path(__file__).resolve().parents[3] / "CHECKSUMS.sha256")
            if (Path(__file__).resolve().parents[3] / "CHECKSUMS.sha256").is_file()
            else None
        ),
...
def main() -> None:
    ...
    if args.resume_from is not None or bool(args.resume_auto):
        raise RuntimeError(
            "Standalone OFIX7-mid candidate resume is disabled by scientific policy."
        )
